  ...
  return (
    <article className="mt-10">
      <Heading className="text-primary !mt-0 border-t-2 pt-8">
        CV Parser Algorithm Deep Dive
      </Heading>
      <Paragraph smallMarginTop={true}>
        For the technically curious, this section will dive into the CareerCraft
        parser algorithm and walks through the 4 steps on how it works. (Note
        that the algorithm is designed to parse single column CVs in English,
        optimized for South African CV standards)
      </Paragraph>
      ...
      <Paragraph>
        And that is everything about the CareerCraft parser algorithm :)
      </Paragraph>
      <Paragraph>
        Written by <Link href="https://github.com/thaboxan">Thabo Jafta</Link> on
        June 2023
      </Paragraph>
    </article>
  );
...
const step4CoreFeatureFunctionTable = [
  ...
  [
    "Phone",
    <>
      Match phone format +27 xx xxx xxxx or 0xx xxx xxxx <br /> Supports South African and international formats
    </>,
    "/\\(?\\d{3}\\)?[\\s-]?\\d{3}[\\s-]?\\d{4}/",
  ],
  [
    "Location",
    <>Match city and province format {"City, Province"}</>,
    "/[A-Z][a-zA-Z\\s]+, [A-Z]{2}/",
  ],
